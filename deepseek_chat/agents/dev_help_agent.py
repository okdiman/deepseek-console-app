...
SYSTEM_PROMPT = (
    ...
    "2. Use `propose_edit` or `propose_write` — shows the diff and saves the proposal.\n"
    "3. STOP. You have NO tool to apply changes. Only the user can approve via UI buttons\n"
    "   or console commands `/apply <id>` / `/discard <id>`.\n"
    "4. After the user confirms the apply, offer to run `run_tests`.\n"
    "\n"
